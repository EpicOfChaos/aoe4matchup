...
const getDockerCommand = (props: WebAppDeploymentProps): string => {
  ...
  if (props.baseDirectory && props.relativeWebAppPath) {
    const dockerOutput = path
      .join('/', 'asset-input', props.relativeWebAppPath, buildDirectory)
      .split(path.sep)
      .join(path.posix.sep)
    return `
...
export class WebAppDeployment extends cdk.Construct {
  constructor(scope: cdk.Construct, id: string, props: WebAppDeploymentProps) {
    ...
    const dockerImage = props.dockerImage || cdk.DockerImage.fromRegistry('aws/codebuild/standard5.0')
      ...
      sources: [
        s3Deploy.Source.asset(buildBaseDirectory, {
          bundling: {
            image: dockerImage,
            entrypoint: ['/bin/sh', '-c'],
            command: ['/bin/sh', '-c', getDockerCommand(props)],
          },
        }),
      ],
